    ...
    class Program
    {
        static void Main(string[] args)
        {
...

            {
                ...
                //TODO
                List<string> str = new List<string> { "1qwerty", "cqwertyc", "cqwe", "c" };
                int amount = str.Aggregate((x, y) => x + y).ToCharArray().Count();
...

            {
                ...
                //TODO
                List<string> str = new List<string> { "aqwerty", "bqwertyc", "cqwe", "d1" };
                string result = string.Concat(str.Select(x => x.First()));
...

            {
                ...
                //TODO
                int k = 7;
                List<string> a = new List<string> { "A7B4C31", "G2R3F1", "T9Y8", "R7U4B0R", "G3O6TT3", "TDD2", "SWS1" };
                var result = a.Where(x => x.Length == k && Regex.IsMatch(x.Last().ToString(), @"[0-9]"));

                Console.Write("\nLinqBegin22: "); int counter = 1;
                foreach (var item in result)
                {
                    Console.Write($"{item}" + (counter == result.Count() ? ". " : ", ")); counter++;
                }
                Console.WriteLine();
            }

            {
                ...
                //TODO
                int d = 8, k = 5;
                IEnumerable<int> a = new int[] { 3, 45, 98, 58, 21, 1, 7, 65, 76, 92, 33, 10 };
                var result = a.Take(d - 1).Union(a.Skip(k - 1)).OrderBy(x => x);

                Console.Write("\nLinqBegin29: "); int counter = 1;
                foreach (var item in result)
                {
                    Console.Write($"{item}" + (counter == result.Count() ? ". " : ", ")); counter++;
                }
...

            {
                ...
                //TODO
                List<string> str = new List<string> { "1qwerty", "cqwertyc", "cqwe", "dfnvkfnk", "9isjdk", "sgnkjsd", "fkfk7" };
                var odd = str.Where(y => y.ToString().Length % 2 != 0).Select(x => x.First());
                var even = str.Where(y => y.ToString().Length % 2 == 0).Select(x => x.Last());
                var result = odd.Concat(even).OrderByDescending(x => x);

                Console.Write("\nLinqBegin36: "); int counter = 1;
                foreach (var item in result)
                {
                    Console.Write($"{item}" + (counter == result.Count() ? ". " : ", ")); counter++;
                }
...

            {
                ...
                IEnumerable<int> b = new int[] { 3, 45, 98, 58, 21, 1, 7, 65, 76, 92, 33, 10 };
                var result = a.Where(x => x > k1).Concat(b.Where(y => y < k2)).OrderBy(z => z);

                Console.Write("\nLinqBegin44: "); int counter = 1;
                foreach (var item in result)
                {
                    Console.Write($"{item}" + (counter == result.Count() ? ". " : ", ")); counter++;
                }
            ...
            }
            {
                ...
                //TODO
                List<string> a = new List<string> { "SW", "SA", "TDD2", "A7B", "4C31", "G2R3F1" };
                List<string> b = new List<string> { "T9Y8", "G3", "O3", "TT3", "R7U4B0R" };
                var result = a.OrderBy(z => z).Join(b.OrderByDescending(z => z),
                    x => x.Length % 10,
                    y => y.Length % 10,
                    (x, y) => x.ToString() + ": " + y.ToString());

                Console.Write("\nLinqBegin48: "); int counter = 1;
                foreach (var item in result)
                {
                    Console.Write($"{item}" + (counter == result.Count() ? ". " : "; ")); counter++;
                }
...

            {
...

                {
                    ...
                    {
                        new Enrollee { SchoolNumber = 144, Year = 2014, LName="Ivanov"},
                        new Enrollee { SchoolNumber = 43, Year = 2015, LName="Petrov"},
                        new Enrollee { SchoolNumber = 188, Year = 2016, LName="Andreev"},
                        new Enrollee { SchoolNumber = 144, Year = 2014, LName="Alekseev"},
                        new Enrollee { SchoolNumber = 118, Year = 2017, LName="Sergeev"},
                        new Enrollee { SchoolNumber = 43, Year = 2016, LName="Petrenko"},
                        new Enrollee { SchoolNumber = 118, Year = 2017, LName="Aleksandrenko"},
                        new Enrollee { SchoolNumber = 118, Year = 2015, LName="Franko"},
                        new Enrollee { SchoolNumber = 78, Year = 2015, LName="Skovoroda"},
                        new Enrollee { SchoolNumber = 43, Year = 2016, LName="Karpenko"}
                    };

                    var schools = enrollees.Select(y => y.SchoolNumber).Distinct();

                    var schYear = enrollees.Select(x => new { Y = x.Year, S = x.SchoolNumber }).Distinct().OrderBy(x => x.S);

                    var schNums = schYear.GroupBy(x => x.Y).Select(x => new { Y = x.Key, S = x.Count() }).
                        OrderBy(x => x.S).ThenBy(y => y.Y);

                    Console.Write("\nLinqObj17: \n");
                    Console.WriteLine(new string('-', 30));

                    foreach (var schNum in schNums)
                    {
                        Console.WriteLine($"Schools number: {schNum.S} Year: {schNum.Y}");
                        Console.WriteLine("Schools №:");

                        foreach (var school in schYear.Where(x => x.Y == schNum.Y))
                            Console.WriteLine($"\t{school.S}");
